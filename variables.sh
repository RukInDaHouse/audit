...
AUDIT_MYSQL_VERSION=$(mysql -e "SHOW GLOBAL VARIABLES" |  grep "^version" | head -1)
...
AUDIT_MYSQL_PORT=$(mysql -e "SHOW GLOBAL VARIABLES" | grep -P "^port" | awk '{print $2}')
# 07. Current MySQL Socket
AUDIT_MYSQL_SOCKET=$(mysql -e "SHOW GLOBAL VARIABLES" | grep -P "^socket" | awk '{print $2}')
...
AUDIT_MYSQL_ERROR_LOG=$(mysql -e "SHOW GLOBAL VARIABLES" | grep -P -m 1 "^log_error" | awk '{print $2}')
...
AUDIT_MYSQL_RELAY_LOG=$(mysql -e "SHOW GLOBAL VARIABLES" | grep ^relay_log | head -1 | awk '{print $2}')
...
AUDIT_MYSQL_VARIABLES_FIRST_TABLE="<tr>"$'\n'"  <td class='cellHeader cellLightMode'>Variable</td>"$'\n'"  <th>MySQL Version</th>"$'\n'"  <th>MySQL Default Engine</th>"$'\n'"  <th>MySQL Basedir</th>"$'\n'"  <th>MySQL Datadir</th>"$'\n'"  <th>MySQL TMP Dir</th>"$'\n'"  <th>MySQL Port</th>"$'\n'"  <th>MySQL Socket</th>"$'\n'"  <th>MySQL PID File</th>"$'\n'"  <th>MySQL Bind Address</th>"$'\n'"</tr>"
AUDIT_MYSQL_VALUES_FIRST_TABLE="<tr>"$'\n'"  <td class='cellHeader cellLightMode'>Value</td>"$'\n'"  <th>$AUDIT_MYSQL_VERSION</th>"$'\n'"  <th>$AUDIT_MYSQL_ENGINE</th>"$'\n'"  <th>$AUDIT_MYSQL_BASEDIR</th>"$'\n'"  <th>$AUDIT_MYSQL_DATADIR</th>"$'\n'"  <th>$AUDIT_MYSQL_TMPDIR</th>"$'\n'"  <th>$AUDIT_MYSQL_PORT</th>"$'\n'"  <th>$AUDIT_MYSQL_SOCKET</th>"$'\n'"  <th>$AUDIT_MYSQL_PID_FILE</th>"$'\n'"  <th>$AUDIT_MYSQL_BIND_ADDRESS</th>"$'\n'"</tr>"

AUDIT_MYSQL_VARIABLES_SECOND_TABLE="<tr>"$'\n'"  <td class='cellHeader cellLightMode'>Variable</td>"$'\n'"  <th>MySQL Log Output</th>"$'\n'"  <th>MySQL General Log File</th>"$'\n'"  <th>MySQL Error Log File</th>"$'\n'"  <th>MySQL Slow Query Log File</th>"$'\n'"  <th>MySQL Relay Log File</th>"$'\n'"  <th>Innodb Max Undo Log Size</th>"$'\n'"  <th>MySQL Expire Logs Days</th>"$'\n'"  <th>Binlog Format</th>"$'\n'"  <th>Innodb Force Recovery Level</th>"$'\n'"</tr>"
AUDIT_MYSQL_VALUES_SECOND_TABLE="<tr>"$'\n'"  <td class='cellHeader cellLightMode'>Value</td>"$'\n'"  <th>$AUDIT_MYSQL_LOG_OUTPUT</th>"$'\n'"  <th>$AUDIT_MYSQL_GENERAL_LOG</th>"$'\n'"  <th>$AUDIT_MYSQL_ERROR_LOG</th>"$'\n'"  <th>$AUDIT_MYSQL_SLOW_LOG</th>"$'\n'"  <th>$AUDIT_MYSQL_RELAY_LOG</th>"$'\n'"  <th>$AUDIT_MYSQL_UNDO_LOG_SIZE</th>"$'\n'"  <th>$AUDIT_MYSQL_EXPIRE_LOGS_DAYS</th>"$'\n'"  <th>$AUDIT_MYSQL_BINLOG_FORMAT</th>"$'\n'"  <th>$AUDIT_MYSQL_FORCE_RECOVERY</th>"$'\n'"</tr>"

AUDIT_MYSQL_VARIABLES_THIRD_TABLE="<tr>"$'\n'"  <td class='cellHeader cellLightMode'>Variable</td>"$'\n'"  <th>Innodb Read Only Status</th>"$'\n'$'\n'"  <th>Default Authentication Plugin</th>"$'\n'"  <th>Innodb Buffer Pool Chunk Size</th>"$'\n'"  <th>Innodb Buffer Pool Size</th>"$'\n'"  <th>Innodb Open Files</th>"$'\n'"  <th>Innodb Buffer Pool Filename</th>"$'\n'"  <th>Innodb Buffer Pool Dump At Shutdown</th>"$'\n'"  <th>Innodb Replication Delay</th>"$'\n'"  <th>Innodb Page Size</th>""</tr>"
AUDIT_MYSQL_VALUES_THIRD_TABLE="<tr>"$'\n'"  <td class='cellHeader cellLightMode'>Value</td>"$'\n'"  <th>$AUDIT_MYSQL_READ_ONLY_STATUS</th>"$'\n'"  <th>$AUDIT_MYSQL_AUTH_PLUGIN</th>"$'\n'"  <th>$AUDIT_MYSQL_CHUNK_SIZE</th>"$'\n'"  <th>$AUDIT_MYSQL_BUFFER_POOL_SIZE</th>"$'\n'"  <th>$AUDIT_MYSQL_OPEN_FILES</th>"$'\n'"  <th>$AUDIT_MYSQL_BUFFER_POOL_FILENAME</th>"$'\n'"  <th>$AUDIT_MYSQL_BUFFER_POOL_DUMP</th>"$'\n'"  <th>$AUDIT_MYSQL_REPL_DELAY</th>"$'\n'"  <th>$AUDIT_MYSQL_PAGE_SIZE</th>"$'\n'"</tr>"
